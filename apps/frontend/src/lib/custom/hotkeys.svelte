...
{:else}
  <div
    class="inline-flex {$$props.class} transfrom ml-2 scale-90 text-zinc-500 {dark
      ? "border bg-zinc-900 text-gray-200"
      : "border-gray-400 bg-zinc-100 text-gray-800"} shadow-outer rounded-sm border font-['Inter']"
  >
    {#each keys as key, i}
      {#if hook}
        <HotKey
